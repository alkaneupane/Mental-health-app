...
export default function Companion() {
  ...
  const startChat = async () => {
    ...
    try {
      ...
      const text = data.content?.find(b => b.type === 'text')?.text || "Hi, I'm here. How are you feeling today?";
      setMessages([{ role: 'assistant', text }]);
    } catch {
      setMessages([{ role: 'assistant', text: "Hi, I'm here for you. How are you feeling today? You can share as much or as little as you'd like." }]);
    }
    setLoading(false);
  };
  ...
  const send = async () => {
    ...
    const hasCrisis = CRISIS_KEYWORDS.some(kw => userText.toLowerCase().includes(kw));
    ...
    try {
      ...
      const data = await res.json();
      const text = data.content?.find(b => b.type === 'text')?.text || "I'm here. Take your time.";
      setMessages(m => [...m, { role: 'assistant', text }]);
    } catch {
      setMessages(m => [...m, { role: 'assistant', text: "I'm still here with you. Sometimes the connection drops, but I haven't gone anywhere." }]);
    }
    setLoading(false);
  };
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ fontSize: 15, color: 'var(--ink-mid)', maxWidth: 340, lineHeight: 1.7 }}>
            A gentle space to talk. I'll listen, reflect, and help you find your way
            — to the community, to resources, or just to feel less alone.
          </p>
        </div>
        <p style={{ fontSize: 12, color: 'var(--ink-soft)', maxWidth: 300 }}>
          This is not therapy. I'm a supportive companion — a first step,
        ...
        <button onClick={startChat} style={{
          ...
          boxShadow: '0 4px 16px rgba(122,158,126,0.35)',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '70vh', maxHeight: 600 }}>
      ...
      {crisisDetected && (
        <div style={{
          background: 'rgba(196,133,106,0.1)', border: '1px solid var(--clay-light)',
          ...
          <div>
            <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--clay)', marginBottom: 4 }}>
              If you're in crisis, please reach out to someone who can help right now.
            </p>
            <p style={{ fontSize: 12, color: 'var(--ink-mid)' }}>
              iCall Nepal: <strong>9152987821</strong> · Crisis Text Line: text <strong>HOME to 741741</strong>
            </p>
          </div>
        </div>
      )}
        ...
        {messages.map((m, i) => (
          <div key={i} style={{
            ...
            <div style={{
              ...
              background: m.role === 'user' ? 'var(--sage)' : 'var(--white)',
        ...
        {loading && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, animation: 'fadeUp 0.3s ease' }}>
            ...
            <div style={{
              ...
              borderRadius: '18px 18px 18px 4px', boxShadow: 'var(--shadow-soft)',
      ...
      <div style={{
        ...
        <input
          ...
          style={{
            ...
            borderRadius: 50, fontSize: 14, color: 'var(--ink)',
          }}
        />
        <button onClick={send} disabled={loading || !input.trim()} style={{
          width: 44, height: 44, borderRadius: '50%',
          background: input.trim() && !loading ? 'var(--sage)' : 'var(--warm-mid)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
